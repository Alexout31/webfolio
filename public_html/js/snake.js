...
const config = {
    ...
    validate() {
        ...
        if (this.settings.rowsCount < 10 || this.settings.rowsCount > 50) {
            result.isValid = false;
            result.errors.push('Неверные настройки, значение rowsCount должно быть в диапазоне [10, 30].');
        }

        if (this.settings.colsCount < 10 || this.settings.colsCount > 50) {
            result.isValid = false;
...
window.onload = game.init({rowsCount: 21, colsCount:21, speed: 5});